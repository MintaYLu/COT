...
- estimate_p_values(): Estimate the p-value for the maximum cos value of each gene. During this step, the distribution of the maximum cos value is fitted to $k$ Gaussian functions, $$f_\text{dist}(x)$$, where $k$ is the number of subtypes. Then the p-value can be estimated as ![formula]("https://render.githubusercontent.com/render/math?math=e^{i \pi} = -1") $p = \int_{x_\text{min}}_{x_\text{max}}{f_\text{dist}(x) dx}$
    - Parameters:
        - N/A
    - Outputs:
